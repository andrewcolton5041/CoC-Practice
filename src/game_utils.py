...
def skill_check(character_data, skill_name):
    ...
    if roll <= extreme_threshold:
        success_level = SUCCESS_EXTREME
    elif roll <= hard_threshold:
        success_level = SUCCESS_HARD
    elif roll <= skill_value:
        success_level = SUCCESS_REGULAR
    elif (skill_value <= FUMBLE_THRESHOLD_STAT and FUMBLE_THRESHOLD_MIN <= roll <= 100) or \
         (skill_value > FUMBLE_THRESHOLD_STAT and roll == 100):
        success_level = SUCCESS_FUMBLE
    else:
        success_level = SUCCESS_FAILURE
...
def opposed_check(char1_data, char1_skill, char2_data, char2_skill):
    ...
    char1_level = SUCCESS_LEVEL_VALUES[char1_success]
    char2_level = SUCCESS_LEVEL_VALUES[char2_success]
...
def roll_damage(weapon_data):
    ...
    try:
        ...
        if DAMAGE_BONUS_PATTERN in damage_formula:
            base_formula = damage_formula.split(DAMAGE_BONUS_PATTERN)[0]

            base_damage = roll_dice(base_formula)
            bonus_damage = roll_dice(DAMAGE_BONUS_DICE)
...
def calculate_skill_check_details(skill_value):
    ...
    return {
        ...
        'hard_success': skill_value // HARD_SUCCESS_DIVISOR,
        'extreme_success': skill_value // EXTREME_SUCCESS_DIVISOR
    }